...
<tr>
	<td>
		<?= Loc::getMessage('USER_TYPE_BOOL_DEFAULT_VALUE') ?>:
	</td>
	<td>
		<select name="<?= $name ?>[DEFAULT_VALUE]">
			<option
				value="1"
...
<tr>
	<td class="adm-detail-valign-top">
		<?= Loc::getMessage('USER_TYPE_BOOL_DISPLAY') ?>:
	</td>
	<td>
		<?php
		foreach($arResult['displays'] as $display)
		{
			...
			<label>
				...
				>
				<?= Loc::getMessage('USER_TYPE_BOOL_' . $display) ?>
			</label>
			<br/>
			<?php
		}
...
<tr>
	<td>
		<?= Loc::getMessage('USER_TYPE_BOOL_LABEL_CHECKBOX') ?>:
	</td>
	<td>
		<input
			type="text"
