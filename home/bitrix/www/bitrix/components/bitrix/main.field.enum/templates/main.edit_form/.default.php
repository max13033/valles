...
elseif($arResult['userField']['SETTINGS']['DISPLAY'] === EnumType::DISPLAY_CHECKBOX)
{
	...
	foreach($arResult['additionalParameters']['items'] as $itemId => $item)
	{
		...
		$fieldName = $arResult['additionalParameters']['NAME'] . '[' . $itemId . ']';
...
elseif($arResult['userField']['SETTINGS']['DISPLAY'] === EnumType::DISPLAY_LIST)
{
	...
	>
		...
		foreach($arResult['additionalParameters']['items'] as $itemId => $item)
		{
			...
			$fullValue = $shortValue = HtmlFilter::encode($item['VALUE']);
			$valueTitle = '';

			if(strlen($fullValue) > EnumUfComponent::MAX_OPTION_LENGTH)
			{
				$textParser = new CTextParser();
				$shortValue = $textParser->html_cut($fullValue, EnumUfComponent::MAX_OPTION_LENGTH);
				$valueTitle = 'title="' . $fullValue . '"';
			}
